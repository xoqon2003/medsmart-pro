...
const PROVIDERS: { id: PaymentProvider; name: string; logo: string; color: string; bg: string; sub: string }[] = [
  { id: 'personal_card', name: 'Shaxsiy karta',  logo: '💳', color: 'from-gray-700 to-gray-900',   bg: 'bg-gray-50 border-gray-200',   sub: 'Karta raqamini kiritib to\'lang'     },
  { id: 'payme',         name: 'Payme',           logo: '🟠', color: 'from-orange-500 to-orange-600',bg: 'bg-orange-50 border-orange-200', sub: 'Karta / Payme hamyon'                },
  { id: 'click',         name: 'Click',           logo: '🔵', color: 'from-cyan-500 to-cyan-600',   bg: 'bg-cyan-50 border-cyan-200',    sub: 'Karta / Click hamyon'                },
  { id: 'uzcard',        name: 'Uzcard',          logo: '🇺🇿', color: 'from-green-600 to-green-700', bg: 'bg-green-50 border-green-200',  sub: 'Milliy to\'lov tizimi'              },
  { id: 'humo',          name: 'Humo',            logo: '💚', color: 'from-emerald-500 to-teal-600', bg: 'bg-emerald-50 border-emerald-200', sub: 'Milliy to\'lov tizimi'            },
  { id: 'uzum',          name: 'Uzum Bank',       logo: '🟣', color: 'from-purple-500 to-purple-600',bg: 'bg-purple-50 border-purple-200', sub: 'Mobil bank orqali to\'lang'        },
  { id: 'cash',          name: 'Naqd pul',        logo: '💵', color: 'from-lime-600 to-green-600',  bg: 'bg-lime-50 border-lime-200',    sub: 'Klinikada kassada to\'lang'          },
];
...
export function Payment() {
  ...
  const [cardCountdown, setCardCountdown] = useState(15 * 60); // 15:00
  const [cardNumberDigits, setCardNumberDigits] = useState('');
  const [expiryDigits, setExpiryDigits] = useState('');

  // Get the latest application created by current user
  const latestApp = applications
    .filter(a => a.patientId === (currentUser?.id || 1))
    .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())[0];
  const arizaNumber = latestApp?.arizaNumber || 'RAD-2026-000000';
  const price = latestApp?.price ?? calculatePrice(draftApplication.serviceType || 'ai_radiolog', draftApplication.urgency || 'normal');

  const isLocked = Boolean(latestApp?.slotLockExpiresAt) && (latestApp?.payment?.status !== 'paid');
  const [lockRemaining, setLockRemaining] = useState<number>(() => {
    const exp = latestApp?.slotLockExpiresAt ? new Date(latestApp.slotLockExpiresAt).getTime() : 0;
  ...
  useEffect(() => {
    ...
    const tick = () => setLockRemaining(Math.max(0, Math.floor((exp - Date.now()) / 1000)));
    tick();
    const t = setInterval(tick, 1000);
    return () => clearInterval(t);
  }, [latestApp?.slotLockExpiresAt]);

  const lockExpired = useMemo(() => isLocked && lockRemaining <= 0, [isLocked, lockRemaining]);

  // Card modal countdown timer (only while modal is open)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Auto-cancel order at 00:00
    updateApplicationStatus(latestApp.id, 'failed', "To'lov muddati tugadi");
    updateApplication(latestApp.id, {
      payment: {
        id: Date.now(),
        applicationId: latestApp.id,
        amount: price,
        provider: 'personal_card',
        status: 'cancelled',
        createdAt: new Date().toISOString(),
      },
    });
    addNotification({
      userId: latestApp.patientId,
      title: 'Buyurtma bekor qilindi',
      message: `${latestApp.arizaNumber} uchun to'lov muddati tugadi (15:00). Ariza avtomatik bekor qilindi.`,
      type: 'warning',
      isRead: false,
      createdAt: new Date().toISOString(),
      applicationId: latestApp.id,
    ...
    goBack();
  }, [addNotification, cardCountdown, goBack, isCardModalOpen, latestApp, price, updateApplication, updateApplicationStatus]);

  useEffect(() => {
    if (!latestApp) return;
    if (!lockExpired) return;
    // Auto-cancel order at 00:00 (global lock timer)
    updateApplicationStatus(latestApp.id, 'failed', "To'lov muddati tugadi");
    updateApplication(latestApp.id, {
      payment: {
        id: Date.now(),
    ...
    // Close any modal and go back
    setIsCardModalOpen(false);
    setIsMethodSheetOpen(false);
    goBack();
  }, [addNotification, goBack, latestApp, lockExpired, price, selectedProvider, updateApplication, updateApplicationStatus]);
  ...
  const handlePay = async () => {
    // Naqd pul — maxsus oqim (klinikada to'lanadi)
    if (selectedProvider === 'cash') {
      ...
      updateApplicationStatus(latestApp.id, 'accepted', 'Naqd pul — klinikada to\'lov');
      setSelectedApplication(latestApp);
      addNotification({
        userId: latestApp.patientId,
        title: 'Bron tasdiqlandi! 📋',
        message: `${latestApp.arizaNumber} — Klinikada naqd to'lov. Shifokor qabulidan keyin kassaga to'lang.`,
        type: 'success',
        isRead: false,
        createdAt: new Date().toISOString(),
        applicationId: latestApp.id,
    ...
    if (success && latestApp) {
      ...
      updateApplicationStatus(latestApp.id, 'paid_pending');
      setSelectedApplication(latestApp);
      addNotification({
        userId: latestApp.patientId,
        title: "To'lov muvaffaqiyatli! ✅",
        message: `${latestApp.arizaNumber} uchun ${formatPrice(latestApp.price)} ${selectedProvider.toUpperCase()} orqali to'landi. Radiolog tez orada ko'rib chiqadi.`,
        type: 'success',
        isRead: false,
        createdAt: new Date().toISOString(),
        applicationId: latestApp.id,
  ...
  const cancelOrder = (reason: string) => {
    ...
    updateApplicationStatus(latestApp.id, 'failed', reason);
    updateApplication(latestApp.id, {
      payment: {
        id: Date.now(),
        applicationId: latestApp.id,
    ...
    addNotification({
      userId: latestApp.patientId,
      title: 'Buyurtma bekor qilindi',
      message: `${latestApp.arizaNumber} bekor qilindi. Sabab: ${reason}`,
      type: 'info',
      isRead: false,
      createdAt: new Date().toISOString(),
      applicationId: latestApp.id,
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center gap-6 px-6">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-2xl p-4 w-full max-w-sm text-center">
          <p className="text-blue-700 text-sm">Radiolog arizangizni ko'rib chiqqanidan so'ng xabarnoma olasiz</p>
        </div>

        <button
          onClick={() => navigate('patient_status')}
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center gap-6 px-6">
        ...
        <div className="text-center">
          ...
          <p className="text-gray-500 text-sm">Karta mablag'i yetarli emas yoki ulanish xatosi</p>
        </div>
        <div className="flex flex-col gap-3 w-full max-w-sm">
          <button
            onClick={() => setPaymentState('select')}
            className="w-full bg-blue-600 text-white rounded-2xl py-3.5"
          >
            Qayta urinish
          </button>
          <button
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
                ...
                {PROVIDERS.map(p => (
                  <button
                    ...
                    className={`w-full p-3.5 rounded-2xl border-2 flex items-center gap-3 transition-all ${
                      selectedProvider === p.id ? `${p.bg} border-opacity-80` : 'border-gray-100 bg-gray-50'
                    }`}
                  >
                    <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${p.color} flex items-center justify-center text-lg flex-shrink-0`}>
                      {p.logo}
                    </div>
                    ...
                    {p.id === 'cash' && (
                      <span className="text-xs bg-lime-100 text-lime-700 px-2 py-0.5 rounded-full font-medium">Klinikada</span>
                    )}
                    {p.id !== 'cash' && <ChevronRight className="w-4 h-4 text-gray-300" />}
                  </button>
                ))}
              </div>
            </motion.div>
            ...
            >
              ...
              <div className="bg-gray-50 rounded-2xl p-4 mb-4">
                <div className="flex justify-between py-1">
                  ...
                  <span className="text-gray-800 text-sm">{formatPrice(price)}</span>
                </div>
                <div className="flex justify-between py-1 border-t border-gray-200/60 mt-2 pt-2">
                  ...
                  <span className="text-gray-900 text-sm">{formatPrice(price)}</span>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <span>To'lov ({formatPrice(price)})</span>
                </button>

                <button
                  onClick={() => cancelOrder("Foydalanuvchi bekor qildi")}
                  className="w-full bg-white border border-gray-200 text-gray-700 rounded-2xl py-3.5"
                >
      ...
      <div className="bg-gradient-to-r from-blue-900 to-indigo-900 pt-12 pb-8 px-5">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h1 className="text-white text-lg">To'lov</h1>
            <p className="text-blue-200 text-xs">5-bosqich: To'lovni amalga oshirish</p>
          </div>
        </div>
        <div className="flex gap-1.5">
          {STEP_LABELS.map((l, i) => (
            <div key={l} className={`h-1 flex-1 rounded-full bg-white`} />
          ))}
        </div>
      </div>

      <div className="flex-1 px-4 py-5 space-y-4 -mt-4 pb-8">
        {/* Global lock timer (15 min) */}
        {isLocked && (
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 flex items-center justify-between">
            ...
            <span className={`text-sm ${lockRemaining < 60 ? 'text-red-600' : 'text-amber-700'}`}>
        ...
        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-2xl p-5 text-center">
          ...
          <p className="text-white text-4xl mb-2">{formatPrice(price)}</p>
        ...
        >
          <CreditCard className="w-5 h-5" />
          <span>To'lovni amalga oshirish ({formatPrice(price)})</span>
        </button>
      </div>
    </div>
  );
